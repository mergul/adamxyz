...
export class NewsListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  // getYPosition(elem: HTMLElement): number {
  //  //   console.log('getYPosition prevOffset --> '+this.prevOffset+' currentOffset --> '+this.currentOffset+' elem.scrollTop --> '+elem.scrollTop)
  //     this.prevOffset = this.currentOffset;
  //     this.currentOffset = elem.scrollTop;
  //     this.isUp = this.prevOffset > this.currentOffset;
  //   //  console.log(' isUp? --> '+this.isUp +' index --> '+this._currLink);
  ...
  //     return this.scrollObs(elem).pipe(takeUntil(this.destroy$)).subscribe(() => {
  //         if (this.activeLink === this.currLink) {
  //             if (this.getYPosition(elem) > (((elem).offsetHeight - 100)
  //                 * (this.currentPage - 1))) {
  //                 this.currentPage++;
  //               //  this.scrChange.emit(of(true));
  //                 this.getStories();
  //             }
  //         }
  //         console.log(' _activeLink --> '+this.activeLink +' _currLink --> '+this.currLink);
  //     });
  // }
