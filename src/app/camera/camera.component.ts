  ...
  connectionOffer = async (msg) => {
    ...
    if (hhj) {
      if (hhj.signalingState !== 'stable') {
        ...
        if (hhj)
          zip(
            hhj.setLocalDescription({ type: 'rollback' }),
            hhj.setRemoteDescription(new RTCSessionDescription(msg.sdp))
          );
},else {
        await hhj.setRemoteDescription(new RTCSessionDescription(msg.sdp));
      }
      if (hut) {
      ...
      await hhj.setLocalDescription(
        await hhj.createAnswer(this.offerOptions).then((answer) => {
      ...
      this.signalingConnection.sendToServer({
        ...
        streams: mystreams,
        event: 'answer',
        sdp: hhj.localDescription,
      });

      // this.localVideo.nativeElement.srcObject = this.merger.result;
      console.log(
        'signaling state at the end of offer response --> ' + hhj.signalingState
      );
    }
  };
