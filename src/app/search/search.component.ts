...
export class SearchComponent implements OnInit, AfterContentInit, OnDestroy {
    ...
    doSearch(term: string): Observable<any[]> {
        ...
        switch (this.searchType) {
            ...
            default: return this.searchService.searchPeople(this.lastTerm);
        }
    }

    onClick(link: any) {
        if(link instanceof EventTarget) this._activeLink = this.links[this.mySelect.nativeElement.selectedIndex];
        else {
