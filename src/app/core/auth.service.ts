...
export class AuthService {
    ...
    loginToGoogle(isMobile: boolean): Observable<User|null> {
        ...
        if (isMobile) {
            return from(this.afAuth.signInWithRedirect(provider)).pipe(switchMap(() => {
            ...
            return from(this.afAuth.signInWithPopup(provider)).pipe(switchMap(credo => {
                this.changeEmitter.next(of(true));
                return of(credo.user);
            }));
        }
    ...
    loginDo(value: any): Observable<User|null> {
        return from(this.afAuth.signInWithEmailAndPassword(value.email, value.password)).pipe(switchMap(credo=>{
            this.changeEmitter.next(of(true));
            return of(credo.user);
        }));
    }
