...
export class HomeComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    track(event){
      //  const elem=document.querySelector('.newst') as HTMLElement;
        const eleme=document.querySelector('.img-hat') as HTMLElement;
        const mas=Math.round(event);
    //    const mast=(event<this.percentage&&((this.percentage-mas)>350||mas<250))?0:mas;
      //  const mes=Math.round(mast/75);
       // console.log('mas --> '+mas+' percentage --> '+(this.percentage));
    //    elem.style.backgroundPosition=mes+'% '+ mes+'%';
     //   elem.style.transition='background-position 0.10s ease-in'
        this.percentage=mas; 
        if (!this.ani) {
            this.ani=eleme.animate({ opacity: ['0.1', '0.5', '0.1'], easing: [ 'ease-in', 'ease-in' ]}, 1500);
            setTimeout(() => {
                this.ani=null;
            }, 2000);
        }
        // this.renderer.setStyle(this.mytags.nativeElement, 'transform', 'translateY('+mas+'px)');
        // this.renderer.setStyle(this.mytags.nativeElement, 'transition', 'transform 0.1s ease-in-out');
    }
    ngOnInit() {
